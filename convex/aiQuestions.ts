...
Please format the entire output strictly as a single JSON array containing these question objects. Do not include any introductory text, explanations, markdown formatting (like \`\`\`json), or anything else outside the JSON array structure. Example format for one question object: {"question": "...", "selectOptions": ["A", "B", "C", "D"], "answer": "B"}`;
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Could not parse LLM response. Expected JSON array format but received different content (Finish Reason: ${result.finishReason}). Response snippet: ${cleanedText.substring(0, 200)}...`);
            }
            ...
            if (!Array.isArray(generatedQuestions)) {
                 ...
                 throw new Error("LLM did not return a valid JSON array of questions.");
            }
            if (generatedQuestions.length === 0 && args.numberOfQuestions > 0) {
                 console.warn("LLM returned an empty array of questions.");
                 throw new Error("LLM returned an empty array. No questions were generated from the file.");
            }
            ...
            return { success: true, count: generatedQuestions.length }; // Return initially parsed count
             ...
             if (error.message?.includes("API key") || error.message?.includes("PERMISSION_DENIED") || error.message?.includes("INVALID_API_KEY")) {
              ...
              } else if (error.cause && typeof error.cause === 'object' && 'status' in error.cause && error.cause.status === 400) {
                   console.error("Bad request error from LLM:", error.cause);
                  throw new Error(`AI model received a bad request (Status 400). Check file format or prompt complexity. Error detail: ${error.cause.message || 'No detail provided'}`);
              }
             throw new Error(`Failed to generate questions via LLM: ${error.message || String(error)}`);
        }
    },
});